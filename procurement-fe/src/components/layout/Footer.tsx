      ...
      <section className="bg-[var(--bg-surface)] border-b border-[var(--border)] py-20 px-6 text-center">
        <div className="max-w-xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link href="/tender" className="flex items-center gap-2 border border-[var(--border-strong)] text-[var(--text-subtle)] hover:text-[var(--text-primary)] hover:border-[var(--text-faint)] text-sm font-medium px-6 py-2.5 rounded-lg transition-colors">
              Browse tenders
            </Link>
          </div>
        </div>
      ...
      <footer className="bg-[var(--bg-base)] border-t border-[var(--border)] py-6 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-[var(--text-primary)]">ProcureHub</span>
          </div>
          <p className="text-xs text-[var(--text-faint)]">
            © {new Date().getFullYear()} ProcureHub. All rights reserved.
          </p>
          <div className="flex items-center gap-4">
            <Link href="/login"        className="text-xs text-[var(--text-faint)] hover:text-[var(--text-subtle)] transition-colors">Login</Link>
            <Link href="/registration" className="text-xs text-[var(--text-faint)] hover:text-[var(--text-subtle)] transition-colors">Register</Link>
            <Link href="/tender"       className="text-xs text-[var(--text-faint)] hover:text-[var(--text-subtle)] transition-colors">Tenders</Link>
          </div>
        </div>
      </footer>
    </>
