...
interface Vendor { id: number; companyName: string; contactPhone?: string | null; address?: string | null; createdAt: string; }
  ...
  return (
    <aside className="hidden lg:flex flex-col shrink-0 border-r border-[var(--border)] bg-[var(--bg-base)] sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto" style={{ width: "18%" }}>
      <div className="p-4 space-y-5">
        <div>
          <p className="text-[10px] font-semibold text-[var(--text-faint)] uppercase tracking-wider px-2 mb-2">Sort</p>
          <nav className="space-y-0.5">
            {([
              ...
              <button key={opt.value} onClick={() => onSortChange(opt.value)}
                className={`w-full flex cursor-pointer items-center gap-2.5 px-2 py-2 rounded-md text-xs transition-colors ${sortBy === opt.value ? "bg-indigo-600/20 text-indigo-300" : "text-[var(--text-subtle)] hover:text-[var(--text-primary)] hover:bg-white/5"}`}>
                <span className={sortBy === opt.value ? "text-indigo-400" : "text-[var(--text-faint)]"}>{opt.icon}</span>
                {opt.label}
              </button>
            ))}
          </nav>
        ...
        {letters.length > 0 && (
          <>
            ...
            <div>
              <p className="text-[10px] font-semibold text-[var(--text-faint)] uppercase tracking-wider px-2 mb-2">Filter A–Z</p>
              <div className="flex flex-wrap gap-1 px-2">
                <button onClick={() => onSearchChange("")} className={`px-2 py-1 rounded text-xs font-medium transition-colors ${search === "" ? "bg-indigo-600/20 text-indigo-300" : "text-[var(--text-faint)] hover:text-[var(--text-primary)] hover:bg-white/5"}`}>All</button>
                {letters.map((l) => (
                  <button key={l} onClick={() => onSearchChange(l)} className={`px-2 py-1 rounded text-xs font-medium transition-colors ${search === l ? "bg-indigo-600/20 text-indigo-300" : "text-[var(--text-faint)] hover:text-[var(--text-primary)] hover:bg-white/5"}`}>{l}</button>
                ))}
              </div>
            </div>
          </>
        ...
        <div className="h-px bg-[var(--border)]" />
        <div>
          <p className="text-[10px] font-semibold text-[var(--text-faint)] uppercase tracking-wider px-2 mb-2">Quick stats</p>
          {[{ label: "Total vendors", value: vendors.length }, { label: "With phone", value: withPhone }, { label: "With address", value: withAddress }].map((row) => (
  ...
  return (
    <aside className="hidden xl:flex flex-col shrink-0 border-l border-[var(--border)] bg-[var(--bg-base)] sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto" style={{ width: "20%" }}>
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-2 px-1"><IconChartBar size={14} className="text-indigo-400" /><p className="text-xs font-semibold text-[var(--text-primary)]">Vendor Analytics</p></div>
        <div className="h-px bg-[var(--border)]" />
        {isLoading ? (
          <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-14 bg-[var(--bg-elevated)] rounded animate-pulse" />)}</div>
        ) : (
          <div className="space-y-3">
            ...
            <div className="px-3 py-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)]">
              ...
              {[{ label: "With phone", count: withPhone, color: "bg-indigo-500" }, { label: "With address", count: withAddress, color: "bg-emerald-500" }].map((item) => (
                <div key={item.label} className="mb-2 last:mb-0">
                  <div className="flex justify-between text-[10px] text-[var(--text-faint)] mb-1"><span>{item.label}</span><span>{vendors.length > 0 ? Math.round((item.count / vendors.length) * 100) : 0}%</span></div>
                  <div className="h-1.5 bg-[var(--bg-elevated)] rounded-full overflow-hidden"><div className={`h-full ${item.color} rounded-full`} style={{ width: `${vendors.length > 0 ? (item.count / vendors.length) * 100 : 0}%` }} /></div>
                </div>
              ))}
            </div>
            {months.length > 0 && (
                ...
                <div className="space-y-2">
                  {[...vendors].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 3).map((v) => (
                    <Link key={v.id} href={`/vendors/${v.id}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                      ...
                      <div className="min-w-0"><p className="text-xs font-medium text-[var(--text-primary)] truncate">{v.companyName}</p><p className="text-[10px] text-[var(--text-faint)]">{new Date(v.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</p></div>
                    </Link>
                  ))}
                </div>
              </div>
