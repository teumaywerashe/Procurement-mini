...
export default function AdminBidTableRow({ bid, tender, isEditing, editingStatus, isUpdating, onEdit, onConfirm, onCancel, onStatusChange }: Props) {
  ...
  return (
    <div className="grid grid-cols-[1fr_120px_150px_1fr] gap-4 px-5 py-4 border-b border-(--border-subtle) hover:bg-(--bg-elevated) transition-colors items-center">
      ...
      {isEditing ? (
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={onCancel}
            className="p-1.5 rounded-md border border-(--border-strong) text-(--text-subtle) hover:text-(--text-primary) transition-colors" title="Cancel">
            <IconX size={12} />
          </button>
        </div>
      ) : (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onEdit(bid)}
            className="p-1 rounded text-(--text-faint) hover:text-indigo-400 hover:bg-indigo-950/40 transition-colors" title="Edit status">
            <IconEdit size={13} />
          </button>
        </div>
      )}
