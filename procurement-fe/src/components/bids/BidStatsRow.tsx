...
export default function BidStatsRow({ total, pending, accepted, rejected }: Props) {
  return (
    <div className="flex  items-center gap-3 mb-5 flex-wrap">
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 bg-yellow-100/30 border border-yellow-800/30 rounded-lg">
        <span className="w-1.5 h-1.5 rounded-full bg-yellow-400" />
        <span className="text-xs text-yellow-400 font-medium">{pending} pending</span>
      </div>
      <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-100/30 border border-emerald-800/30 rounded-lg">
        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
        <span className="text-xs text-emerald-400 font-medium">{accepted} accepted</span>
      </div>
      <div className="flex items-center gap-2 px-3 py-1.5 bg-red-100/30 border border-red-800/30 rounded-lg">
