...
function BidCard({ bid, label, color }: BidCardProps) {
  ...
  return (
    <Link
      ...
      className="block px-3 py-3 rounded-lg bg-(--bg-surface) border border-(--border) hover:border-indigo-500/40 transition-colors space-y-1.5"
    >
      <div className="flex items-center justify-between gap-1">
        <span
          className={`text-[9px] font-semibold uppercase tracking-wider ${color}`}
        >
          {label}
        </span>
        <span
          className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${s?.bg} ${s?.text}`}
        >
          {bid.bidStatus}
        </span>
      </div>
      ...
      {tender?.name && (
        <p className="text-[10px] text-(--text-faint)">{tender.name}</p>
      )}
    </Link>
  );
}
...
export default function AdminBidsRightSidebar({ bids, isLoading }: Props) {
  const { highest, lowest } = React.useMemo(() => {
    ...
    bids.forEach((b) => {
      if (
        !highMap[b.tenderId] ||
        Number(b.amount) > Number(highMap[b.tenderId].amount)
      )
        highMap[b.tenderId] = b;
      if (
        !lowMap[b.tenderId] ||
        Number(b.amount) < Number(lowMap[b.tenderId].amount)
      )
        lowMap[b.tenderId] = b;
    });
    return {
      highest: Object.values(highMap).sort(
        (a, b) => Number(b.amount) - Number(a.amount),
      ),
      lowest: Object.values(lowMap).sort(
