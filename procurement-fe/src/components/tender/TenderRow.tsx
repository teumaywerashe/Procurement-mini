...
import { IconFileText, IconCurrencyDollar, IconClock } from "@tabler/icons-react";
    ...
    >
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        <div>
          ...
          <p className="text-xs sm:text-sm text-(--text-subtle) mt-0.5">{tender.name}</p>
        </div>
        ...
        {tender.description && (
          <p className="text-xs text-(--text-subtle) line-clamp-1">{tender.description}</p>
        )}
