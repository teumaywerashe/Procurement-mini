...
export default function VendorBidSection({ tender, closing }: Props) {
  ...
  if (vendorLoading || myBidsLoading) return (
    <div className="mt-6 bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl px-8 py-5">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-[var(--bg-elevated)] animate-pulse" />
        <div className="space-y-2"><div className="h-3 w-32 bg-[var(--bg-elevated)] rounded animate-pulse" /><div className="h-2.5 w-48 bg-[var(--bg-elevated)] rounded animate-pulse" /></div>
      </div>
    </div>
  );
        ...
        {!hasVendorProfile ? (
          showVendorForm ? (
            <form onSubmit={handleCreateVendor} className="space-y-4">
              <div><h2 className="text-sm font-semibold text-[var(--text-primary)]">Create Vendor Profile</h2><p className="text-xs text-[var(--text-subtle)] mt-0.5">You need a vendor profile before you can submit bids.</p></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={field}>
                    <label className="block text-xs font-medium text-[var(--text-subtle)] mb-1">{label}{required && <span className="text-red-400"> *</span>}</label>
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <button type="button" onClick={() => setShowVendorForm(false)} className="px-4 py-2.5 rounded-lg border border-[var(--border-strong)] text-sm text-[var(--text-subtle)] hover:text-[var(--text-primary)] transition-colors">Cancel</button>
              </div>
            </form>
          ) : (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm font-semibold text-[var(--text-primary)]">Vendor profile required</p><p className="text-xs text-[var(--text-subtle)] mt-0.5">You need a vendor profile to submit bids.</p></div>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-semibold text-emerald-400">Bid Submitted</p><p className="text-xs text-[var(--text-subtle)] mt-0.5">You have already submitted a bid. <Link href="/bids/my" className="text-indigo-400 hover:text-indigo-300 transition-colors">View my bids →</Link></p></div>
          </div>
        ) : !canBid ? (
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[var(--bg-elevated)] flex items-center justify-center"><IconGavel size={18} className="text-[var(--text-faint)]" /></div>
            <div><p className="text-sm font-medium text-[var(--text-subtle)]">Bidding not available</p><p className="text-xs text-[var(--text-faint)] mt-0.5">This tender is {tender.status} and is not accepting bids.</p></div>
          </div>
        ) : showBidForm ? (
          <form onSubmit={handleSubmitBid}>
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="button" onClick={() => setShowBidForm(false)} className="px-4 py-2.5 rounded-lg border border-[var(--border-strong)] text-sm text-[var(--text-subtle)] hover:text-[var(--text-primary)] transition-colors">Cancel</button>
            </div>
          </form>
        ) : (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm font-semibold text-[var(--text-primary)]">Submit a Bid</p><p className="text-xs text-[var(--text-subtle)] mt-0.5">Estimated value: ${Number(tender.estimatedValue).toLocaleString()}</p></div>
            </div>
