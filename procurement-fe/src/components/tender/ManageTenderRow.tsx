...
export default function ManageTenderRow({ tender, now, onDelete }: Props) {
  ...
  const actions = (
    <div className="flex items-center gap-1.5">
      <Link href={`/tender/${tender.id}`} className="p-2 text-(--text-subtle) hover:text-(--text-primary) rounded-md hover:bg-(--bg-elevated) border border-transparent hover:border-(--border) transition-all" title="View">
    ...
    <div className="border-b border-(--border-subtle) hover:bg-(--bg-elevated) transition-colors px-5 py-4">
      ...
      <div className="sm:hidden flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-(--text-primary) truncate">{tender.title}</p>
          <p className="text-[11px] text-(--text-faint) mt-0.5">{tender.referenceNumber}</p>
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            ...
            <span className="text-xs font-medium text-(--text-muted)">${Number(tender.estimatedValue).toLocaleString()}</span>
            <span className={`flex items-center gap-1 text-xs ${closing === "Closed" ? "text-(--text-faint)" : isUrgent ? "text-orange-400" : "text-(--text-subtle)"}`}>
              <IconClock size={11} />{closing === "Closed" ? "Closed" : `${closing} left`}
            </span>
          </div>
        </div>
      ...
      <div className="hidden sm:grid grid-cols-[1fr_130px_120px_130px_110px] gap-3 items-center">
        ...
        <span className="text-sm font-medium text-(--text-muted)">${Number(tender.estimatedValue).toLocaleString()}</span>
        <span className={`flex items-center gap-1 text-xs ${closing === "Closed" ? "text-(--text-faint)" : isUrgent ? "text-orange-400" : "text-(--text-subtle)"}`}>
