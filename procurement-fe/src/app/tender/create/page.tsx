...
export default function CreateTenderPage() {
  ...
  const [form, setForm] = React.useState<FormState>({
    title: "", name: "Infrastructure", description: "", status: "draft", closingDate: "", estimatedValue: "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await createTender({
      title: form.title, name: form.name, description: form.description || undefined,
      status: form.status, closingDate: new Date(form.closingDate).toISOString(),
      estimatedValue: Number(form.estimatedValue),
    });
    if ("data" in result && result.data) {
      notifications.show({ title: "Tender Created", message: "Tender has been created successfully.", color: "green" });
          ...
          <form onSubmit={handleSubmit} className="px-8 py-6 space-y-5">
            ...
            {error && (
              <div className="text-xs text-red-400 bg-red-900/20 border border-red-800/40 rounded-lg px-4 py-2.5">
                {(() => {
                  const data = ("data" in error ? error.data : null) as { message?: string | string[] } | null;
                  const raw = data?.message;
                  const msgs = Array.isArray(raw) ? raw : raw ? [raw] : ["Failed to create tender."];
