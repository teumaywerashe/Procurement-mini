    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-6 max-w-sm w-full">
        ...
        <p className="text-sm text-[var(--text-subtle)] mb-5 leading-relaxed">
          ...
          <span className="text-[var(--text-primary)] font-medium">{tender.title}</span>. This cannot be undone.
        </p>
...
export default function ManageTendersPage() {
  ...
  const stats = {
    ...
    closingSoon: tenders.filter((t) => {
      const d = Math.ceil((new Date(t.closingDate).getTime() - Date.now()) / 86_400_000);
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className={statusFilter === f.value ? "text-indigo-400" : "text-[var(--text-faint)]"}>
                          {f.icon}
                        </span>
                        {f.label}
                      </span>
                      <span className={`text-[10px] font-semibold tabular-nums px-1.5 py-0.5 rounded-full ${
                        statusFilter === f.value ? "bg-indigo-600/30 text-indigo-300" : "bg-[var(--bg-elevated)] text-[var(--text-faint)]"
                      }`}>
                        {count}
                      </span>
                    </button>
              ...
              {isLoading ? (
                <div className="p-6 space-y-3">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="h-12 bg-[var(--bg-elevated)] rounded animate-pulse" />
                  ))}
                </div>
              ) : isError ? (
                ...
                <div className="flex flex-col items-center py-16 gap-3">
                  <IconFileText size={28} className="text-[var(--text-faint)]" />
                  <p className="text-sm text-[var(--text-subtle)]">No tenders found.</p>
                </div>
              ) : (
                filtered.map((t) => {
                  const s = STATUS_COLORS[t.status] ?? STATUS_COLORS.draft;
                  const closing = daysLeft(t.closingDate);
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-[var(--text-faint)] mt-0.5">{t.referenceNumber}</p>
                      </div>
                      ...
                      <span className={`flex items-center gap-1 text-xs ${
                        closing === "Closed" ? "text-[var(--text-faint)]"
                ...
                {stats.total > 0 && (
                  <div className="px-3 py-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)]">
                    ...
                    <div className="flex h-2 rounded-full overflow-hidden gap-0.5">
                      {stats.published > 0 && <div className="bg-emerald-500" style={{ width: `${(stats.published / stats.total) * 100}%` }} />}
                      {stats.draft > 0 && <div className="bg-zinc-500" style={{ width: `${(stats.draft / stats.total) * 100}%` }} />}
                      {stats.awarded > 0 && <div className="bg-indigo-500" style={{ width: `${(stats.awarded / stats.total) * 100}%` }} />}
                      {stats.closed > 0 && <div className="bg-red-500" style={{ width: `${(stats.closed / stats.total) * 100}%` }} />}
                      {stats.cancelled > 0 && <div className="bg-orange-500" style={{ width: `${(stats.cancelled / stats.total) * 100}%` }} />}
                    </div>
                  </div>
                )}
              </div>
