  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)] flex flex-col">
      ...
      <div className="flex flex-1 w-full overflow-hidden">
        ...
        <main className="flex-1 min-w-0 overflow-y-auto">
          <div className="px-6 py-6">
            ...
            {!isLoading && filtered.length > 0 && (
              <BidStatsRow total={filtered.length} pending={pending} accepted={accepted} rejected={rejected} />
            )}
            ...
            <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl overflow-hidden">
              ...
              {isLoading ? (
                <div className="p-6 space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-14 bg-[var(--bg-elevated)] rounded animate-pulse" />)}</div>
              ) : isError ? (
                ...
                <AdminBidTableRow
                  key={bid.id} bid={bid} tender={tenderMap[bid.tenderId]}
                  isEditing={editingId === bid.id} editingStatus={editingStatus}
                  isUpdating={isUpdating}
                  onEdit={(b) => { setEditingId(b.id); setEditingStatus(b.bidStatus); }}
                  onConfirm={confirmEdit} onCancel={() => setEditingId(null)}
        ...
        <AdminBidsRightSidebar bids={bids} tenderMap={tenderMap} isLoading={isLoading} />
      </div>
    </div>
  );
}
