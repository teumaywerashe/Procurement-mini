...
export default function MyBidsPage() {
  ...
  const { data: bids = [], isLoading, isError } = useGetBidsByVendorQuery(vendor?.id ?? 0, {
    skip: !vendor?.id,
  });
      ...
      <div className="flex flex-1 w-full overflow-hidden">
        ...
        <main className="flex-1 min-w-0 overflow-y-auto">
          <div className="px-6 py-6">
            <div className="mb-5">
              <h1 className="text-lg font-bold text-[var(--text-primary)]">My Bids</h1>
              <p className="text-xs text-[var(--text-subtle)] mt-0.5">Track the status of your submitted bids</p>
            </div>
            ...
            {!isLoading && bids.length > 0 && (
              <div className="flex items-center gap-3 mb-5 flex-wrap">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg">
                  ...
                  <span className="text-xs font-bold text-[var(--text-primary)] tabular-nums">{bids.length}</span>
                </div>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center py-16 gap-3">
                  ...
                  <p className="text-sm text-[var(--text-subtle)]">
                    {bids.length === 0 ? "You haven't submitted any bids yet." : "No bids match the filter."}
                  </p>
                  {bids.length === 0 && (
                    <Link href="/tender" className="flex items-center gap-1.5 text-xs font-medium text-indigo-400 hover:text-indigo-300 transition-colors mt-1">
        ...
        >
          <div className="p-4 space-y-4">
            <div className="flex items-center gap-2 px-1">
              ...
              <p className="text-xs font-semibold text-[var(--text-primary)]">Bid Statistics</p>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                ...
                <div className="px-3 py-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)]">
                  <p className="text-[10px] text-[var(--text-faint)] mb-1">Success rate</p>
                ...
                {bids.length > 0 && (
                  <div className="px-3 py-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)]">
                    <p className="text-[10px] text-[var(--text-faint)] mb-2">Status breakdown</p>
                    <div className="flex h-2 rounded-full overflow-hidden gap-0.5">
                      {accepted > 0 && (
                        <div
                          className="bg-emerald-500 rounded-full"
                          style={{ width: `${(accepted / bids.length) * 100}%` }}
                      ...
                      {rejected > 0 && (
                        <div
                          ...
                          style={{ width: `${(rejected / bids.length) * 100}%` }}
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      {[
                        { label: "Accepted", color: "bg-emerald-500", count: accepted },
                        { label: "Pending",  color: "bg-yellow-500",  count: pending  },
                        { label: "Rejected", color: "bg-red-500",     count: rejected },
                      ].map((item) => (
                        <div key={item.label} className="flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${item.color}`} />
                          <span className="text-[10px] text-[var(--text-faint)]">{item.label}: {item.count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {bids.length === 0 && (
                  <div className="text-center py-4">
                    <IconTrendingUp size={24} className="text-[var(--text-faint)] mx-auto mb-2" />
                    <p className="text-xs text-[var(--text-subtle)]">Submit bids to see statistics.</p>
                  </div>
                )}
              </div>
            )}
